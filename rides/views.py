...
def get_not_rated_rides(request, user_id):
    if is_authorized(request):
        try:
            ...
            # switch to email
            reviewer = User.objects.get(user_id=1)
        ...
        rides_with_user = Ride.objects.filter(driver=user, passengers__in=[reviewer])
        rides_with_user_and_reviewer = rides_with_user.filter(driver=reviewer, passengers__in=[user])
